...
strataa_metaphlan_data_mat <- strataa_metaphlan_data |> 
  as.matrix() |>
  t()

dist_mat <- vegdist(strataa_metaphlan_data_mat)
cmd_res <- cmdscale(dist_mat, 
                   k = (nrow(strataa_metaphlan_data_mat) - 1),
                   eig = TRUE)
...
pcoa_df <- mutate(pcoa_df, SampleID = rownames(strataa_metaphlan_data_mat)) %>% left_join(strataa_metaphlan_metadata, by = 'SampleID')


View(pcoa_df)

p <- ggplot(pcoa_df, aes(x = PC1, y = PC2, colour = Group)) + geom_point()
p
