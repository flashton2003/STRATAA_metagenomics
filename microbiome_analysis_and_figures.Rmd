...
strataa_metaphlan_data_l <- strataa_metaphlan_data_cln %>% pivot_longer(!feature, names_to = 'sample', values_to = 'abundance') %>% left_join(strataa_metaphlan_metadata %>% mutate(sample = rownames(strataa_metaphlan_metadata)), by = 'sample')
...
```{r get_same_same}
heal_ac_car_taxonomic_all <- filter_taxonomic_maaslin(heal_ac_car_taxonomic)
heal_ac_car_taxonomic_all_w <- heal_ac_car_taxonomic_all %>% pivot_wider(names_from = value, values_from = c(coef, qval), id_cols = c(feature, lowest_taxonomic_level))
same_same <- heal_ac_car_taxonomic_all_w %>% filter(qval_Acute_Typhi >= 0.05 & qval_Carrier >= 0.05) %>% mutate(type = 'same_same')
