...
baseline_chars_table$Carrier <- replace(tmp$Carrier,which(tmp$Carrier==0),NA)
baseline_chars_table$`Healthy Control` <- replace(tmp$`Healthy Control`,which(tmp$`Healthy Control`==0),NA)
...
```{r}
all_countries_beta <- metaphlan_beta(strataa_metaphlan_data, strataa_metaphlan_metadata, c('Bangladesh', 'Malawi', 'Nepal'))
all_countries_beta$pn_res %>% kbl %>% kable_styling()
# all_countries_beta_plot$pc12
# all_countries_beta_plot$pc34

bgd_beta <- metaphlan_beta(strataa_metaphlan_data, strataa_metaphlan_metadata, c('Bangladesh'))
bgd_beta$pn_res %>% kbl %>% kable_styling()
# bgd_beta_plot$pc12
# bgd_beta_plot$pc34

mal_beta <- metaphlan_beta(strataa_metaphlan_data, strataa_metaphlan_metadata, c('Malawi'))
mal_beta$pn_res %>% kbl %>% kable_styling()

# mal_beta_plot$pc12
# mal_beta_plot$pc34

nep_beta <- metaphlan_beta(strataa_metaphlan_data, strataa_metaphlan_metadata, c('Nepal'))
# nep_beta_plot
nep_beta$pn_res %>% kbl %>% kable_styling()

bgd_beta$pc12 / mal_beta$pc12 / nep_beta$pc12
...
```{r}
# filter strataa_metaphlan_metadata to get the samples from Bangladesh
# then select those from the strataa_metaphlan_data


```

## Beta diversity - EdgeR
