...
all_countries_all_groups_alpha <- metaphlan_alpha(strataa_metaphlan_data_species, metadata, countries_of_interest = c('Bangladesh', 'Malawi', 'Nepal'), groups_of_interest = c('Acute typhoid', 'Carrier', 'Control'), comparisons = list(c('Acute typhoid', 'Carrier'), c('Acute typhoid', 'Control'), c('Control', 'Carrier')), participant_group_colours)
...
all_countries_beta_all_groups <- strataa_metaphlan_beta(strataa_metaphlan_data_species, metadata, c('Bangladesh', 'Malawi', 'Nepal'), c('Acute typhoid', 'Carrier', 'Control'), participant_group_colours)
all_countries_beta_all_groups$pn_res %>% kbl %>% kable_styling()

bgd_beta_all_groups <- strataa_metaphlan_beta(strataa_metaphlan_data_species, metadata, c('Bangladesh'), c('Acute typhoid', 'Carrier', 'Control'), participant_group_colours)
bgd_beta_all_groups$pn_res %>% kbl %>% kable_styling()
# bgd_beta_all_groups$pc12
# bgd_beta_all_groups$pc34

mal_beta_all_groups <- strataa_metaphlan_beta(strataa_metaphlan_data_species, metadata, c('Malawi'), c('Acute typhoid', 'Carrier', 'Control'), participant_group_colours)
mal_beta_all_groups$pn_res %>% kbl %>% kable_styling()
mal_beta_all_groups$pc12
# mal_beta_all_groups$pc34

nep_beta_all_groups <- strataa_metaphlan_beta(strataa_metaphlan_data_species, metadata, c('Nepal'), c('Acute typhoid', 'Carrier', 'Control'), participant_group_colours)
...
phyla_clean_metadata <- prep_data_to_plot_phyla(phyla, metadata)

order_of_groups <- c("Acute typhoid", "Control")
...
all_countries_healthy_acute_alpha <- metaphlan_alpha(strataa_metaphlan_data_species, metadata, countries_of_interest = c('Bangladesh', 'Malawi', 'Nepal'), groups_of_interest = c('Acute typhoid', 'Control'), comparisons = list(c('Acute typhoid', 'Control')), participant_group_colours = participant_group_colours)
...
malawi_healthy_acute_alpha <- metaphlan_alpha(strataa_metaphlan_data_species, metadata, countries_of_interest = c('Malawi'), groups_of_interest = c('Acute typhoid', 'Control'), comparisons = list(c('Acute typhoid', 'Control')), participant_group_colours)
...
all_countries_beta_acute_healthy <- strataa_metaphlan_beta(strataa_metaphlan_data_species, metadata, c('Bangladesh', 'Malawi', 'Nepal'), c('Acute typhoid', 'Control'), participant_group_colours)
all_countries_beta_acute_healthy$pn_res %>% kbl %>% kable_styling()

bgd_beta_acute_healthy <- strataa_metaphlan_beta(strataa_metaphlan_data_species, metadata, c('Bangladesh'), c('Acute typhoid', 'Control'), participant_group_colours)
bgd_beta_acute_healthy$pn_res %>% kbl %>% kable_styling()

mal_beta_acute_healthy <- strataa_metaphlan_beta(strataa_metaphlan_data_species, metadata, c('Malawi'), c('Acute typhoid', 'Control'), participant_group_colours)
mal_beta_acute_healthy$pn_res %>% kbl %>% kable_styling()

nep_beta_acute_healthy <- strataa_metaphlan_beta(strataa_metaphlan_data_species, metadata, c('Nepal'), c('Acute typhoid', 'Control'), participant_group_colours)
...
Setting up the forest plots.

```{r forest_plots_setup}
# i want to join bangladesh_taxonomic_maaslin, malawi_taxonomic_maaslin, nepal_taxonomic_maaslin, and the patch data.
# should be based on 
...
# give these columns a "_nep" suffix coef	stderr	N	N.not.0	pval	qval
combined_maaslins <- combined_maaslins %>%
  rename_with(~ paste0(.x, '_nep'), c("coef", "stderr", "N", "N.not.0", "pval", "qval"))

# get the patch data

patch_taxonomic_maaslin <- read_tsv('/Users/flashton/Dropbox/GordonGroup/STRATAA_Microbiome/patch_data/metaphlan/results/2024.01.10/baseline_typhi_species/all_results.tsv')
...
combined_maaslins <- combined_maaslins %>%
  left_join(patch_taxonomic_maaslin, by = c("feature", 'metadata', 'value', 'lowest_taxonomic_level'))
 

combined_maaslins 
pcopri <- combined_maaslins %>% filter(grepl('_Prevotella_copri_clade_A', lowest_taxonomic_level)) %>% filter(metadata == 'Group') %>% select(!c(feature, metadata, value))
...
# pcopri <- pcopri %>% select(lowest_taxonomic_level, country, coef, stderr, qval)

# in pcopri, change the country column entries from 'bang' to 'Bangladesh', 'mal' to 'Malawi', and 'nep' to 'Nepal' and from 'patch' to 'CHIM'
pcopri$country <- ifelse(pcopri$country == 'bang', 'Bangladesh', pcopri$country)
pcopri$country <- ifelse(pcopri$country == 'mal', 'Malawi', pcopri$country)
pcopri$country <- ifelse(pcopri$country == 'nep', 'Nepal', pcopri$country)
pcopri$country <- ifelse(pcopri$country == 'patch', 'CHIM', pcopri$country)

desired_order <- c("Bangladesh", "Malawi", "Nepal", "CHIM")
pcopri$country <- factor(pcopri$country, levels = desired_order)
pcopri$country <- as.character(pcopri$country)
...
for the forest plot, i also want to include the per-cohort abundance IQRs for the species of interest.

```{r species_of_interest_abundance}
...
pcopri_species <- prevalence %>% filter(grepl('_Prevotella_copri_clade_A', lowest_taxonomic_level)) %>% select(!c(feature, variable))
# i want to pivot pcopri_species wider
pcopri_species_prevalence <- pcopri_species %>% pivot_wider(names_from = 'value', values_from = 'Median_Prevalence')

pcopri <- pcopri %>% left_join(pcopri_species_prevalence, by =c("lowest_taxonomic_level", 'country'))
```
...
tabletext <- cbind(
  country = pcopri$country,
  lowest_taxonomic_level = pcopri$lowest_taxonomic_level,
  coef = pcopri$coef,
  stderr = pcopri$stderr,
  qval = pcopri$qval,
  N = pcopri$N,
  N.not.0 = pcopri$N.not.0,
  abundunce_disease = pcopri$`Acute typhoid`,
  abundunce_control = pcopri$Control
)

mean <- as.numeric(pcopri$coef)
lower <- as.numeric(pcopri$coef) - 1.96 * as.numeric(pcopri$stderr)
upper <- as.numeric(pcopri$coef) + 1.96 * as.numeric(pcopri$stderr)

x_min <- min(lower) - 1  # Extend range slightly for better visualization
x_max <- max(upper) + 1
...
```{r print_pcopri_results, eval = FALSE}
...
combined_results$positive_coef %>%
  dplyr::select(!c(metadata, value, N_bang, N.not.0_bang, pval_bang, N_mal, N.not.0_mal, pval_mal)) %>% 
...
combined_results$positive_coef %>% mutate(split_species = str_split(Species, '_')) %>% mutate(genus = sapply(split_species, "[[", 1)) %>% mutate(specific = sapply(split_species, "[[", 2)) %>% mutate(clean_species = paste(genus, specific, sep = ' ')) %>% dplyr::select(-split_species, -genus, -specific) %>% relocate(clean_species, .after = Species) %>% group_by(MGC_class) %>% arrange(Species) %>% summarise(n = n(), Species = paste(clean_species, collapse = ', ')) %>% arrange(desc(n), MGC_class) %>%  kbl() %>% kable_styling()
...
all_countries_healthy_carrier_alpha <- metaphlan_alpha(strataa_metaphlan_data_species, metadata, countries_of_interest = c('Malawi', 'Nepal'), groups_of_interest = c('Carrier', 'Control'), comparisons = list(c('Carrier', 'Control')), participant_group_colours)
...
all_countries_beta_carrier_healthy <- strataa_metaphlan_beta(strataa_metaphlan_data_species, metadata, c('Malawi', 'Nepal'), c('Carrier', 'Control'), participant_group_colours)
all_countries_beta_carrier_healthy$pn_res %>% kbl %>% kable_styling()

# bgd_beta_carrier_healthy <- strataa_metaphlan_beta(strataa_metaphlan_data_species, metadata, c('Bangladesh'), c('Carrier', 'Control'))
# bgd_beta_carrier_healthy$pn_res %>% kbl %>% kable_styling()

mal_beta_carrier_healthy <- strataa_metaphlan_beta(strataa_metaphlan_data_species, metadata, c('Malawi'), c('Carrier', 'Control'), participant_group_colours)
mal_beta_carrier_healthy$pn_res %>% kbl %>% kable_styling()

nep_beta_carrier_healthy <- strataa_metaphlan_beta(strataa_metaphlan_data_species, metadata, c('Nepal'), c('Carrier', 'Control'), participant_group_colours)
...
all_countries_healthy_acute_carrier_alpha <- metaphlan_alpha(strataa_metaphlan_data_species, metadata, countries_of_interest = c('Malawi', 'Nepal'), groups_of_interest = c('Carrier', 'Control', 'Acute typhoid'), comparisons = list(c('Acute typhoid', 'Carrier'), c('Acute typhoid', 'Control'), c('Carrier', 'Control')), participant_group_colours)
...
all_countries_beta_carrier_healthy <- strataa_metaphlan_beta(strataa_metaphlan_data_species, metadata, c('Malawi', 'Nepal'), c('Acute typhoid', 'Carrier', 'Control'), participant_group_colours)
all_countries_beta_carrier_healthy$pn_res %>% kbl %>% kable_styling()

# bgd_beta_carrier_healthy <- strataa_metaphlan_beta(strataa_metaphlan_data, metadata, c('Bangladesh'), c('Carrier', 'Control'))
# bgd_beta_carrier_healthy$pn_res %>% kbl %>% kable_styling()

mal_beta_acute_carrier_healthy <- strataa_metaphlan_beta(strataa_metaphlan_data_species, metadata, c('Malawi'), c('Acute typhoid', 'Carrier', 'Control'), participant_group_colours)
...
nep_beta_acute_carrier_healthy <- strataa_metaphlan_beta(strataa_metaphlan_data_species, metadata, c('Nepal'), c('Acute typhoid', 'Carrier', 'Control'), participant_group_colours)
...
mwi_sgb_abundance <- strataa_metaphlan_data_mwi_sp %>% group_by(sample, SGB) %>% summarise(total_abundance = sum(abundance)) %>% left_join(metadata %>% mutate(sample = rownames(metadata)), by = 'sample')  %>% mutate(SGB = ifelse(SGB, 'SGB', 'non-SGB')) %>% mutate(Group = factor(Group, levels = c('Control', 'Acute typhoid', 'Carrier')))
