...
tmp <- amr_data_typhi_meta %>% group_by(sample, Class) %>% summarise(n = n(), sum_reads_per_kb = sum(reads_per_kb)) #%>% kbl() %>% kable_styling()

# 3. join the data to the template (amr_summarised_typhi)
amr_summarised_typhi_d <- left_join(amr_summarised_typhi, tmp, by = join_by(sample == sample, Class == Class)) %>% mutate(n = ifelse(is.na(n), 0, n)) %>% mutate(sum_reads_per_kb = ifelse(is.na(sum_reads_per_kb), 0, sum_reads_per_kb)) %>% mutate(at_least_one = ifelse(n > 0, 1, 0)) 

# 4. join to metadata
amr_summarised_typhi_d_meta <- left_join(amr_summarised_typhi_d, select(metadata, c(Lane, Country, Group)), by = join_by(sample == Lane))

tmp <- amr_summarised_typhi_d_meta %>% filter(Class == 'MACROLIDE')

# 5. Group by class, country, and summarise
typhi_amr_class_country <- amr_summarised_typhi_d_meta %>% 
  group_by(Class, Country) %>% 
  summarise(n_samples = n(), n_samples_with_gene = sum(at_least_one), proportion_samples_with_gene = n_samples_with_gene / n_samples, n_genes = sum(n), median_reads_per_kb = median(sum_reads_per_kb)) #%>% 
  # kbl() %>% kable_styling()

run_chisq_on_amr <- function(typhi_amr_class_country_df, antibiotic_class){
  test <- typhi_amr_class_country_df %>% 
    ungroup() %>% 
    filter(Class == antibiotic_class) %>% 
    select(Country, n_samples_with_gene, n_samples) %>% 
...
# 5. Group by class, country, group, and summarise
amr_summarised_typhi_d_meta %>% 
  group_by(Class, Country, Group) %>% 
  summarise(n_samples = n(), n_samples_with_gene = sum(at_least_one), proportion_samples_with_gene = n_samples_with_gene / n_samples, n_genes = sum(n), sum_reads_per_kb = sum(sum_reads_per_kb)) #%>% 
  # kbl() %>% kable_styling()
