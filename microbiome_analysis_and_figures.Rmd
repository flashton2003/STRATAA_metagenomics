...
strataa_metaphlan_metadata <- strataa_metaphlan_metadata %>% mutate(Group = if_else(Group == 'Control', 'Control', Group)) %>% mutate(Group = if_else(Group == 'Acute typhoid', 'Acute typhoid', Group))
...
all_countries_all_groups_alpha <- metaphlan_alpha(strataa_metaphlan_data_species, strataa_metaphlan_metadata, countries_of_interest = c('Bangladesh', 'Malawi', 'Nepal'), groups_of_interest = c('Acute typhoid', 'Carrier', 'Control'), comparisons = list(c('Acute typhoid', 'Carrier'), c('Acute typhoid', 'Control'), c('Control', 'Carrier')))
...
# todo - do the overall graphs for healthy vs acute vs carrier (?)
...
tmp <- amr_data_typhi_meta %>% group_by(sample, Class) %>% summarise(n = n()) #%>% kbl() %>% kable_styling()
...
set.seed(12343)  # For reproducibility
train_index <- sample(1:nrow(strataa_metaphlan_data_heal_carr_rf), 0.8*nrow(strataa_metaphlan_data_heal_carr_rf))
train_data <- strataa_metaphlan_data_heal_carr_rf[train_index, ]
test_data <- strataa_metaphlan_data_heal_carr_rf[-train_index, ]

rf_model <- randomForest(Group ~ ., data=train_data, ntree=1500)

predictions <- predict(rf_model, test_data)
confusionMatrix <- table(test_data$Group, predictions)
...
CLR transform, makes no difference.

```{r rf_clr}

# Adding a small constant to avoid log(0)
strataa_metaphlan_data_adj <- strataa_metaphlan_data_t %>% select(-sample_name)
strataa_metaphlan_data_adj <- strataa_metaphlan_data_adj + 1e-6

# Calculating the geometric mean for each row
geom_mean <- apply(strataa_metaphlan_data_adj, 1, function(x) exp(mean(log(x))))

# Applying the CLR transformation
clr_transformed <- sweep(strataa_metaphlan_data_adj, 1, geom_mean, FUN = "/")
clr_transformed <- log(clr_transformed)

clr_transformed$sample_name <- rownames(clr_transformed)


clr_transformed_heal_carr_rf <- clr_transformed %>% left_join(strataa_metaphlan_metadata_copy, by = "sample_name") %>% filter(Group != "Acute typhoid", Country != 'Bangladesh') %>% mutate(Group = as.factor(Group))
row.names(clr_transformed_heal_carr_rf) <- clr_transformed_heal_carr_rf$sample_name
clr_transformed_heal_carr_rf <- clr_transformed_heal_carr_rf %>% select(-sample_name) %>% filter(!is.na(Group))
...
train_index <- sample(1:nrow(clr_transformed_heal_carr_rf), 0.8*nrow(clr_transformed_heal_carr_rf))
train_data <- clr_transformed_heal_carr_rf[train_index, ]
test_data <- clr_transformed_heal_carr_rf[-train_index, ]
...
Using a strategy from one of Kevin Bonham's papers https://www.biorxiv.org/content/10.1101/2020.02.13.944181v5.full
