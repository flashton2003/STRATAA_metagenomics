...
plot_species_of_interest <- function(prevalence_meta, species_of_interest, country_of_interest, groups_of_interest){
  ...
  abundance_compared <- compare_means(prevalence ~ Group, data = prevalence_meta_filtered, group.by = "age_bracket")
  p <- ggplot(prevalence_meta_filtered, aes(x = age_bracket, y = prevalence, fill = factor(Group))) + 
    ...
    annotate("text", x = 3, y = 47, label = paste0('p = ', toString(abundance_compared[,6]$p.adj)))
  # p <- ggplot(prevalence_meta_filtered, aes(x = age_bracket, y = prevalence)) + geom_boxplot()
  show(p)
  return(abundance_compared)

}

a <- plot_species_of_interest(strataa_metaphlan_data_longer_meta, 's__Prevotella_copri_clade_A', 'Malawi', c('Acute_Typhi', 'Control_HealthySerosurvey'))
