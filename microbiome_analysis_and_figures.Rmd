...
metadata_for_plotting$Group <- factor(metadata_for_plotting$Group, levels = c("Healthy Control", "Acute typhoid", "Carrier"))
...
all_countries_beta_carrier_healthy <- metaphlan_beta(strataa_metaphlan_data, strataa_metaphlan_metadata, c('Malawi', 'Nepal'), c('Acute_Typhi', 'Carrier', 'Control_HealthySerosurvey'))
all_countries_beta_carrier_healthy$pn_res %>% kbl %>% kable_styling()

# bgd_beta_carrier_healthy <- metaphlan_beta(strataa_metaphlan_data, strataa_metaphlan_metadata, c('Bangladesh'), c('Carrier', 'Control_HealthySerosurvey'))
# bgd_beta_carrier_healthy$pn_res %>% kbl %>% kable_styling()

mal_beta_acute_carrier_healthy <- metaphlan_beta(strataa_metaphlan_data, strataa_metaphlan_metadata, c('Malawi'), c('Acute_Typhi', 'Carrier', 'Control_HealthySerosurvey'))
...
nep_beta_acute_carrier_healthy <- metaphlan_beta(strataa_metaphlan_data, strataa_metaphlan_metadata, c('Nepal'), c('Acute_Typhi', 'Carrier', 'Control_HealthySerosurvey'))
...
number_of_amr_genes_per_sample_group$Group <- factor(number_of_amr_genes_per_sample_group$Group, levels = c("Healthy Control", "Acute typhoid", "Carrier"))
...
p <- ggplot(number_of_amr_genes_per_sample_group, aes(x = Group, y = n)) +
  ...
  geom_jitter(alpha = 0.1, height = 0) +
  stat_compare_means() + 
  stat_compare_means(comparisons = list(c('Healthy Control', 'Acute typhoid'), c('Healthy Control', 'Carrier'), c('Acute typhoid', 'Carrier'))) + 
  rremove("x.text") + 
  rremove("xlab") + 
  ylab('Number of AMR genes identified per sample') +
  rotate_x_text(angle = 45)
...
amr_summarised_meta_with_data$Group <- factor(amr_summarised_meta_with_data$Group, levels = c("Healthy Control", "Acute typhoid", "Carrier"))
...
subclasses <- c('BETA-LACTAM', 'CEPHALOSPORIN', 'CARBAPENEM')
# 2. make an empty data frame, with just the samples and classes
...
amr_summarised_meta_with_data_betalac$Group <- factor(amr_summarised_meta_with_data_betalac$Group, levels = c("Healthy Control", "Acute typhoid", "Carrier"))
