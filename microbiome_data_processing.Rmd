...
run_calculate_alpha <- function(output_folder, tax_level, countries, fn_regex, braken_folder, meta){
  ...
  anova_res <- calculate_alpha(filtered_otu.unnormalised, meta_subset, quote("Group"), output_folder, "Phenotype")  
  anova_res <- anova_res[[1]] %>% mutate(significant = ifelse(`Pr(>F)` < 0.01, 'significant', 'not_significant')) %>% arrange(desc(significant), `Pr(>F)`)

  anova_res %>% kbl(digits = 2) 
}
