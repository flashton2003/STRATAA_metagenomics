...
amr_summarised <- expand.grid(sample = samples, drug_class = classes)
...
amr_data_with_ncbi_cln_meta_summarised$Group <- factor(amr_data_with_ncbi_cln_meta_summarised$Group, levels = c("Control_HealthySerosurvey", "Acute_Typhi", "Carrier"))

classes <- unique(amr_data_with_ncbi_cln_meta_summarised$Class)
...
for (drug_class in classes) {
    drug_class_data <- amr_data_with_ncbi_cln_meta_summarised %>% filter(Class == drug_class)
...
for (drug_class in classes) {
    ...
    p <- ggboxplot(drug_class_data, facet.by = "Country", y = "sum_reads_per_kb", x = "Group", color = "Group") + 
      stat_compare_means() + 
      rremove("x.text") + 
      rremove("xlab") + 
      rotate_x_text(angle = 45) +
...
amr_data_with_ncbi_cln_meta_betalactams_summarised <- amr_data_with_ncbi_cln_meta %>% filter(Class == 'BETA-LACTAM') %>% group_by(sample, Subclass) %>% summarise(n = n(), sum_reads_per_kb = sum(reads_per_kb)) %>% left_join(metadata, by = join_by(sample == Lane))
amr_data_with_ncbi_cln_meta_betalactams_summarised$Group <- factor(amr_data_with_ncbi_cln_meta_betalactams_summarised$Group, levels = c("Control_HealthySerosurvey", "Acute_Typhi", "Carrier"))
subclasses <- c('BETA-LACTAM', 'CEPHALOSPORIN')

for (drug_class in subclasses) {
    drug_class_data <- amr_data_with_ncbi_cln_meta_betalactams_summarised %>% filter(Subclass == drug_class)
    p <- ggboxplot(drug_class_data, facet.by = "Country", y = "n", x = "Group", color = "Group") + 
      stat_compare_means() + 
      rremove("x.text") + 
      rremove("xlab") + 
