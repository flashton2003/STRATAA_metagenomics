...
metadata_subset <- patch_metadata %>% 
  select(Run, Patient, Gender, Group, Species, Visit, Day, day_group, Diagnosis, Dose, SampleID, pre_post_antibiotics) %>% 
...
Maaslin2(input_data = metaphlan_data_baseline, input_metadata = baseline_metadata_typhi, analysis_method = "LM", min_prevalence = 0,
           normalization  = 'NONE',
           transform = 'LOG',
           output         = file.path(patch_maaslin_taxonomic_output_root_folder, 'baseline2'), 
           fixed_effects  = variables_for_analysis)

#reference = c("sequencing_lane,32580_7", "Group,Typhi", "Run,run_1")
```
...
rownames(patch_bigmap) <- rownames(patch_bigmap) %>% str_replace_all('-', '_')

variables_for_analysis <- c("Gender", "Group", "Diagnosis", "Run")
Maaslin2(input_data = patch_bigmap, input_metadata = baseline_metadata_typhi, analysis_method = "LM", min_prevalence = 0,
           normalization  = 'NONE',
           transform = 'LOG',
           output         = file.path('/Users/flashton/Dropbox/GordonGroup/STRATAA_Microbiome/patch_data/big-map/2023.05.11', 'baseline2'), 
           fixed_effects  = variables_for_analysis,
