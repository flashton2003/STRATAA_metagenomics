...
bgd_mal_nep_taxonomic_maaslin_acute_healthy_filtered <- bgd_mal_nep_taxonomic_maaslin_acute_healthy_filtered %>% left_join(patch_taxonomic_maaslin, by = c("feature", 'metadata', 'value', 'lowest_taxonomic_level'))
...
malawi_sgbs <- strataa_metaphlan_data_species %>%
    ...
    filter(str_detect(feature, 'SGB')) %>% select(-Group, -Country) 
    # mutate(SGB = ifelse(str_detect(feature, 'SGB'), 'SGB', 'non-SGB')) 

malawi_sgbs <- malawi_sgbs %>% pivot_wider(names_from = sample, values_from = abundance) %>% as.data.frame()

# row.names(tmp2) <- tmp2$feature
# tmp2 <- tmp2 %>% select(-feature)
