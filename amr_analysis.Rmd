...
samples <- metadata %>% filter(!(Group == 'Carrier')) %>% pull(Lane)
...
samples_with_carrier <- metadata %>% filter(!(Country == 'Bangladesh' & Group == 'Carrier')) %>% pull(Lane)
...
# 4. join to metadata
amr_summarised_typhi_carrier_d_meta <- left_join(amr_summarised_typhi_carrier_d, select(metadata, c(Lane, Country, Group)), by = join_by(sample == Lane))
...
amr_summarised_typhi_d_meta_abu <- left_join(amr_summarised_typhi_d, select(metadata, c(Lane, Country, Group, Antibiotics_taken_before_sampling)), by = join_by(sample == Lane))  %>% filter(Country == 'Bangladesh') %>% filter(Group == 'Acute_Typhi')

amr_summarised_typhi_d_meta_abu <- amr_summarised_typhi_d_meta_abu %>% mutate(Azithromyin = ifelse(str_detect(Antibiotics_taken_before_sampling, 'Azithromycin'), 1, 0)) %>% mutate(Ciprofloxacin = ifelse(str_detect(Antibiotics_taken_before_sampling, 'Ciprofloxacin'), 1, 0)) %>% mutate(Cefixime = ifelse(str_detect(Antibiotics_taken_before_sampling, 'Cefixime'), 1, 0))

amr_summarised_typhi_d_meta_abu_quinolone <- filter(amr_summarised_typhi_d_meta_abu, Class == 'QUINOLONE')

amr_summarised_typhi_d_meta_abu_azithromycin <- filter(amr_summarised_typhi_d_meta_abu, Class == 'MACROLIDE')

amr_summarised_typhi_d_meta_abu_bl_esbl <- filter(amr_summarised_typhi_d_meta_abu, Class == 'ESBL' | Class == 'BETA-LACTAM')
