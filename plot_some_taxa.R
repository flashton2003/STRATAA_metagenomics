...
data_table.normalised.clr <- pivot_longer(data_table.normalised.clr, !taxon)


data_table.normalised.clr <- data_table.normalised.clr %>% left_join(select(meta, c('ID', 'Group', 'Country')), by = c('name' = 'ID'))

healthy_taxa <- c('Prevotella.copri', 'Dialister.sp000434475')
typhi_taxa <- c( 'Absiella.innocuum', 'Pauljensenia.odontolyticus', 'KLE1796.sp001580115', 'Massiliomicrobiota.timonensis')


data_table.normalised.clr <- filter(data_table.normalised.clr, Country != 'Nepal')

data_table.normalised.clr <- filter(data_table.normalised.clr, Group != 'Carrier')
healthy_all <- filter(data_table.normalised.clr, taxon %in% healthy_taxa)
typhi_all <- filter(data_table.normalised.clr, taxon %in% typhi_taxa)

p_healthy_all <- ggplot(healthy_all, aes(x = taxon, y = value, fill = factor(Group))) + geom_boxplot() + coord_flip() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=0.5)) + coord_flip() + ylab('CLM transformed reads') + xlab('Species')
p_typhi_all <- ggplot(typhi_all, aes(x = taxon, y = value, fill = Group)) + geom_boxplot() + coord_flip() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=0.5)) + coord_flip() + ylab('CLM transformed reads') + xlab('Species')
